function validateFormAdd() {
  ...
  let getImg = document.querySelector("#urlImg") as HTMLInputElement;
  ...
  // Kiểm lỗi tải hình ảnh
  const file = getImg.files?.[0];
  if (file == undefined) {
    getImg.classList.remove("showTrue");
    getImg.classList.add("showFalse");
    err2 = "Vui lòng tải lên hình ảnh !";
    check = false;
  } else {
    getImg.classList.add("showTrue");
    getImg.classList.remove("showFalse");
    showErr2.innerHTML = "";
  }
  if (err2 != "") {
    showErr2.innerHTML = err2;
  }
